...
def require_confidence(min_confidence=40):
    """Confidence threshold guard for signal execution."""
    def decorator(func):
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
            conf = kwargs.get("confidence")

            if conf is None and len(args) > 1:
                conf = args[1]

            if conf is None:
                logger.warning(
                    f"[BLOCKED] {func.__name__}: confidence missing"
                )
                return None

            if conf < min_confidence:
                logger.info(
                    f"[BLOCKED] {func.__name__}: confidence {conf} < {min_confidence}"
                )
                return None
