from datetime import datetime, timezone


class IntelligenceRuntimeV11_73AutonomousRuntimeSupervisor:

    VALID_DECISIONS = {
        "ALLOW",
        "BLOCK",
    }

    def __init__(self, runtime_control):

        self.runtime_control = runtime_control
        self.supervisor_status = "INITIALIZED"
        self.last_decision = None
        self.supervision_cycles = 0
        self.decision_history = []

    def _time(self):
        return datetime.now(timezone.utc).isoformat()


    def supervise_runtime(self):

        self.supervision_cycles += 1

        record = {
            "cycle": self.supervision_cycles,
            "started_at": self._time(),
        }

        try:

            control = (
                self.runtime_control.control_runtime()
            )

            decision = control.get(
                "decision",
                "BLOCK"
            )


            if decision not in self.VALID_DECISIONS:
                decision = "BLOCK"


            self.last_decision = decision


            if decision == "ALLOW":
                self.supervisor_status = "ACTIVE"
            else:
                self.supervisor_status = "BLOCKED"


            record["decision"] = decision
            record["status"] = "SUCCESS"
            record["completed_at"] = self._time()

            self.decision_history.append(record)


            return {
                "supervision_status": self.supervisor_status,
                "runtime_status": (
                    "ACTIVE"
                    if self.supervisor_status == "ACTIVE"
                    else "ERROR"
                ),
                "decision": decision,
                "control": control,
                "supervision_cycles": self.supervision_cycles,
                "checked_at": self._time(),
            }


        except Exception as error:

            self.supervisor_status = "ERROR"
            self.last_decision = "BLOCK"

            record["status"] = "FAILED"
            record["error"] = str(error)

            self.decision_history.append(record)


            return {
                "supervision_status": "FAILED",
                "runtime_status": "ERROR",
                "decision": "BLOCK",
                "error": str(error),
                "checked_at": self._time(),
            }



    def health(self):

        dependency_health = None

        if hasattr(
            self.runtime_control,
            "health"
        ):

            try:
                dependency_health = (
                    self.runtime_control.health()
                )

            except Exception:

                dependency_health = {
                    "runtime_status": "FAILED"
                }


        return {

            "autonomous_runtime_supervisor_available": True,

            "supervisor_status":
                self.supervisor_status,

            "runtime_status":
                (
                    "ACTIVE"
                    if self.supervisor_status == "ACTIVE"
                    else "ERROR"
                ),

            "last_decision":
                self.last_decision,

            "supervision_cycles":
                self.supervision_cycles,

            "decision_history_size":
                len(self.decision_history),

            "dependency_health":
                dependency_health,

            "checked_at":
                self._time(),
        }



    def stop(self):

        self.supervisor_status = "STOPPED"

        return {
            "supervisor_status": "STOPPED",
            "runtime_status": "STOPPED",
            "stopped_at": self._time(),
        }
